...
class GPTLMLoss_meta(nn.Module):
    """
    ...
    def __init__(self, ring_attn_group=None):
        ...
        self.loss = nn.CrossEntropyLoss(ignore_index=self.IGNORE_INDEX, reduction='none')  # 修改返回的loss向量
    ...
    def forward(self, logits: torch.Tensor, labels: torch.Tensor, alpha: float, sp_token_ids: list) -> torch.Tensor:
        # RingAttention
        if self.ring_attn_group is not None:
            ...
            # 修改了关键的loss
            loss_per_token = self.loss(flat_logits, flat_labels)
            
            # 创建权重张量，初始化为1.0
            weights = torch.ones_like(flat_labels, dtype=torch.float32)

            # 确定特殊token的位置（排除被忽略的索引）
            # special_tokens = [151646, 151647, 151648, 151649, 151650]
            special_tokens_tensor = torch.tensor(sp_token_ids, device=flat_labels.device)
            is_special = torch.isin(flat_labels, special_tokens_tensor)

            # true_count = is_special.sum().item() 检查数量
            non_ignore_mask = (flat_labels != self.IGNORE_INDEX)
            valid_special_mask = is_special & non_ignore_mask

            # 应用双倍权重
            weights[valid_special_mask] = alpha

            # 计算加权损失
            weighted_loss = loss_per_token * weights

            # 统计有效token数（排除被忽略的）
            num_valid = non_ignore_mask.sum()

            # 避免除以零
            if num_valid > 0:
                loss = weighted_loss.sum() / num_valid
            else:
                loss = weighted_loss.sum()  # 返回0
...
class GPTLMmtLoss(nn.Module):
    ...
    def forward(self, logits: torch.Tensor, labels: torch.Tensor, impts: torch.Tensor) -> torch.Tensor:
        # RingAttention
        if self.ring_attn_group is not None:
            total_seq_len = labels.size(-1)
            seq_len_per_process = total_seq_len // self.ring_attn_world_size
            ...
            shift_logits = logits[..., :-1, :].contiguous()
            shift_labels = labels[..., 1:].contiguous()
            shift_impts = impts[..., 1:].contiguous()
            ...
            flat_logits = shift_logits.view(-1, shift_logits.size(-1))
            flat_labels = shift_labels.view(-1)
            flat_impts = shift_impts.view(-1)
            # true_count = is_special.sum().item() 检查数量
            valid_mask  = (flat_labels != self.IGNORE_INDEX)
            flat_impts = flat_impts * valid_mask  # 去除无效位置的权重

            # 计算loss
            loss = self.loss(flat_logits, flat_labels)
            # 将损失与重要性权重相乘
            weighted_loss = loss * flat_impts
            
            # 统计有效token数（排除被忽略的）
            # num_valid = valid_mask .sum()+ 1e-8
            # loss = weighted_loss.sum() / num_valid
            
            # 符合“重要性权重”的直觉（即权重越高对损失贡献越大）。
            denominator = flat_impts.sum() + 1e-8
...
class PRMLoss(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, logits: torch.Tensor, labels: torch.Tensor, *, return_acc: bool = False):
        ...
        if labels.dtype == torch.float:
            # soft label 
            # 因为对应的labels不再是索引了, 而是在+ -token维度的分布, 此时衡量CE衡量正负token的分布的差异,
            ...
            # this is slow....
            # 效果
            # labels-before   tensor([10, 10, 10, 10, 12, 12, 10, 12], device='cuda:0')
            # labels-after    tensor([0, 0, 0, 0, 1, 1, 0, 1], device='cuda:0')
        ...
        # acc表示PRM当前能够进行正确预测的概率, 理论上经过训练, 这个acc会不断升高, 也就是可以准确判断结果了
